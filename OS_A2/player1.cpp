...
{
 do{
  cout << "Enter the valid guess index of the ship: (row column)\n";
  cin >> row >> col;
  } while (row < 0 || row >= SIZE || col < 0 || col >= SIZE);
...
int main()
{
  ...
  int w=open("player1",O_WRONLY);
  int r=open("player2",O_RDONLY);
  int grid1[SIZE][SIZE];
  initializeGrid(grid1);
  for(int i=0;i<NUM;i++)
  {
    int size=SHIPS[i];
    getShipPosition(grid1,size);
  }
  cout<<"Player 1 Grid:\n";
  printGrid(grid1);
  cout<<"----------GAME STARTS----------\n";
